...
class BatteryControl:
    def __init__(self, inverter_sn=None):
        ...
        print(f"BATTERY CONTROL initialized dry_run={self.dry_run} inverter_sn={self._safe(self.inverter_sn)}", flush=True)
    ...
    def _safe(self, value):
        if value is None:
            return ""
        value = str(value)
        return "****" if len(value) <= 4 else value[:2] + "****" + value[-2:]
    ...
    def _sign(self, body, date, endpoint):
        ...
        encrypt_str = "POST" + "\n" + md5_value + "\n" + self.content_type + "\n" + date + "\n" + endpoint
        signature = base64.b64encode(hmac.new(self.key_secret.encode("utf-8"), encrypt_str.encode("utf-8"), hashlib.sha1).digest()).decode("utf-8")
    ...
    def _post(self, endpoint, payload, token=None):
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}{endpoint}", headers=headers, data=body, timeout=20)
    ...
    def login(self):
        ...
        data = self._post("/v2/api/login", {"userInfo": self.user_name, "passWord": md5_password})
    ...
    def _is_error_value(self, value):
        ...
        if text == "":
            return True
        error_keywords = [
            "communication error", "please refresh", "try again later", "data error",
            "datalogger returns data abnormally", "forbidden", "not found",
            "too many requests", "error", "failed",
        ]
    ...
    def _normalize_time_slot(self, value):
        ...
        return text if self._looks_like_time_slot(text) else "00:00-00:00"
    ...
    def read_cid_once(self, cid):
        if not self.inverter_sn:
            ...
            return None
        self._throttle_read()
        response = self._post("/v2/api/atRead", {"inverterSn": self.inverter_sn, "cid": int(cid)})
    ...
    def dry_run_apply_inhibit_plan(self):
        ...
        if not self.inverter_sn:
            print("BATTERY CONTROL inhibit plan skipped: missing inverter SN", flush=True)
            return False
        ...
        if cid_636 is not None and str(cid_636) != "1":
            apply_payloads.append({"description": "Enable Storage Inverters Control Switching", "cid": "636", "inverterSn": self.inverter_sn, "value": "1", "yuanzhi": str(cid_636), "language": self.language})
            restore_payloads.insert(0, {"description": "Restore Storage Inverters Control Switching", "cid": "636", "inverterSn": self.inverter_sn, "value": str(cid_636), "yuanzhi": "1", "language": self.language})
    ...
    def validate_solis_charge_discharge_settings(self, force=False):
        ...
        if marker is not None:
            ...
            if str(marker).strip().lower() in ["0xaa55", "aa55", "43605"]:
    ...
    def read_6972_value_from_unit_cids(self):
        ...
        for idx in range(6):
            ...
            if None in [switch_value, time_slot, current, soc, voltage]:
                ...
                return None
            groups.append([self._normalize_switch(switch_value), self._normalize_time_slot(time_slot), self._normalize_numeric(current, "0"), self._normalize_numeric(soc, "20"), self._normalize_numeric(voltage, "48")])
        for idx in range(6):
    ...
    def inhibit_discharge(self, mode="manual", duration_h=None):
        ...
        if not new_value:
            ...
            return
        payload = {"cid": str(self.cid_charge_discharge_one_cid), "inverterSn": self.inverter_sn, "value": new_value, "yuanzhi": self.last_6972_value, "language": self.language}
        if self.dry_run:
    ...
    def resume_discharge(self, mode="manual"):
        ...
        payload = {"cid": str(self.cid_charge_discharge_one_cid), "inverterSn": self.inverter_sn, "value": self.last_6972_value, "yuanzhi": current_yuanzhi, "language": self.language}
        if self.dry_run:
