...
def main():
    ...
    while True:
        try:
            ...
            if solis_data:
                last_good_solis_data = merge_with_defaults(solis_data, DEFAULT_SOLIS_DATA)
                print("Solis data OK", flush=True)
            else:
                print("No Solis data received, using last known values", flush=True)

            safe_solis_data = merge_with_defaults(last_good_solis_data, DEFAULT_SOLIS_DATA)
            ...
            safe_ai_data = merge_with_defaults(ai_data, DEFAULT_AI_DATA)
            payload = {**safe_solis_data, **safe_ai_data}
            ...
            fallback_payload = {**DEFAULT_SOLIS_DATA, **DEFAULT_AI_DATA}
            try:
